...
from mcj.runtime.mapping import dynamic_mapping, key_mapping, no_op
from mcj.runtime.profiles import (
    FeedbackConfig,
...
from mcj.runtime.states import DefinitionState, PromptState, TrialState
...
def build_practice_profile_config() -> TaskProfileConfig[CJAction]:
    return TaskProfileConfig(
        termination_by_state={
            ...
            TrialState.STIMULUS: ActionOrTimeoutTermination(
                {CJAction.LEFT, CJAction.RIGHT}
            ),
...
def build_experiment_profile_config(test: bool = False) -> TaskProfileConfig[CJAction]:
    return TaskProfileConfig(
        termination_by_state={
            ...
            DefinitionState.DEFINITION: TimeTermination(),
            TrialState.FIXATION: TimeTermination(),
        ...
        action_mapping_by_state={
            ...
            DefinitionState.DEFINITION: no_op(),
            TrialState.FIXATION: no_op(),
            TrialState.STIMULUS: dynamic_mapping(build_action_mapping),
            TrialState.FEEDBACK: no_op(),
        },
        timing=TimingConfig(
            ...
            definition_duration_seconds=12.0 if not test else 2.0,
